...
source("scripts/phydynR_and_MCMC/load_data.R")
source("scripts/phydynR_and_MCMC/model1.2_phydynR.R")
...
load("scripts/phydynR_and_MCMC/iter.rdata")

while(i < 801){
  if(!file.exists("out_sim.RDS")){
    #when using the HPC I use the option of parallel = 3
    #bayesianSetup3 <- createBayesianSetup(likelihood = co0 , prior = prior,
    #                                      names = ESTNAMES , parallel = 3)
    bayesianSetup3 <- createBayesianSetup(likelihood = co0 , prior = prior,
                                          names = ESTNAMES , parallel = FALSE)
